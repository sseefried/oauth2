...
module RackOauth2
  # RackOauth2::Lint validates your application and the requests and
  ...
  class Lint
    ...
    ## can (and should) use RackOauth2::Lint to enforce it.
    ##
      ...
      headers.each { |key, value|
        ...
        if key.downcase == "content-type"
          assert("Content-Type header found in #{status} response, not allowed") {
            not RackOauth2::Utils::STATUS_WITH_NO_ENTITY_BODY.include? status.to_i
          }
          return
        end
      }
      assert("No Content-Type header found") {
        RackOauth2::Utils::STATUS_WITH_NO_ENTITY_BODY.include? status.to_i
      }
    end
    ...
    def each
      ...
      @body.each { |part|
        ...
        bytes += RackOauth2::Utils.bytesize(part)
        yield part
      }
